...
`problem` contains derived arithmetic plus an optional sampled curriculum tag. Query-forced diagnostic problems deliberately have no tag. `state` contains temporary interaction progress, including the active beetle prompt key and template. A session-only map prevents an immediate repeat when a prompt type is selected again. Profile schema v3 contains durable score, milestone progress, sound, difficulty mode, current level, current-level success count, higher-level success count, schema version, and update timestamp. Keeping these separate prevents rendered CSS classes from becoming a source of truth.

Profile data is written to IndexedDB and mirrored to a compact cookie. The newest valid copy wins at startup; if storage is unavailable, the game continues with in-memory state. Schema-v1 records migrate to L1 with zero advancement counters while preserving score, milestones, sound, and timestamp. Schema-v2 records preserve those fields plus their curriculum level and both counters. Both migrations select the new `standard` default. Schema-v3 validation accepts only `standard` or `easy`. Reset creates a fresh L1 v3 profile while carrying sound and difficulty forward. There is no backend, account, telemetry, or cross-device synchronization.
