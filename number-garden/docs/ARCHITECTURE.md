...
- **Interaction controllers:** event handlers advance manual counting, recognize the default 1.5-second quick-play hold on the current addend cell, start Tutorial from the unfinished step, collect typed answers, change problems, refresh only the three alternative cards, reset state, toggle sound/fullscreen, change difficulty without replacing the active problem, override levels, and manage settings. Internal solver function names and DOM IDs remain stable for compatibility.
- **Feedback services:** reusable Web Audio helpers produce optional drop, heavy quick-drop impact, carry, beetle, outcome, celebration, and synthesized dice-rattle effects. The dice sound combines filtered noise bursts with short impacts and requires no media asset. Fixed-position overlay clones animate between measured source/result rectangles without becoming interactive; quick-drop impact overlays shake and pulse over the destination cell. An ordered ARIA live queue announces landed counts and phase changes.
