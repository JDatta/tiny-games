...
Multiplication uses the same horizontal Hundreds/Tens/Ones board as addition, with operand and result badges in the first column. The visible rows show the first number, `×` and the second number, then `=` and the hidden or completed answer; Multiplicand, Multiplier, and Product remain the accessible role names. The multiplicand row remains visible and cannot be moved. Each multiplier One pulls a clone of every non-empty multiplicand place into the Product row, after which Ones regroup horizontally into a gold Ten bar and Tens regroup to Hundreds automatically and visibly. The latest regrouped Ten stays gold until the next Ones-to-Tens regroup, when the prior bar becomes green. A multiplier Ten must be tapped explicitly to convert it horizontally into ten waiting Ones. Holding the multiplier Ones cell starts Pull All for that active group only; pulls settle sequentially, and the learner must tap each later Ten separately. Multiplier Tens never batch, and multiplication never exposes Drop All. Product Hundreds render one through five flats as a clean, non-overlapping vertical column whenever the cell has room. If the cell is too short—or the count is six through nine—the flats compact into a vertical stack with a required `×N` badge while retaining every underlying unit.
